...
function YearModelSelection(props) {
  ...
  return (
    ...
    <div style={{marginLeft:20}}><h5 style={{ display: "inline-block", marginLeft: 0,marginRight:3,fontSize:16,fontWeight:600}}>Models:</h5>{props.all_models.map(item => <p className={props.defualt_models.includes(item) ? "years_p_selected years_p" : "years_p"} onClick={() => handle_model_click(item)}>{item}</p>)}</div>
    </div>
  );
}
