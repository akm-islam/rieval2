...
export default function create_top_explanation_plot(parent_id, selected_instances, sorted_features, lime_data, selected_year, defualt_models, 
    ...
    d3.select("#" + parent_id).selectAll(".features").data(sorted_features).join("svg").attr("class", "features")
        ...
        .attr("svg1_add_feature_exp",function(d){
            d3.select(this).selectAll("#svg1").data([0]).join("svg").attr('id','svg1').attr("x", 0).attr("width", feature_width).attr("height", top_parent_height)
        ...
        .attr("svg2_add_feature_hist",function(d){
            d3.select(this).selectAll("#svg2").data([0]).join("svg").attr('id','svg2').attr("x", feature_width).attr("width", top_histogram_width).attr("height", top_parent_height)
            .attr("add_exp_circles",function(){
                CreatexpHistogram(d,d3.select(this), selected_instances, sorted_features, lime_data, selected_year, defualt_models, clicked_circles, Set_clicked_circles, diverginColor, anim_config, clicked_features, Set_clicked_features)
            })
        })
}